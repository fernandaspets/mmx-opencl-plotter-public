...
// GPU-resident variant: takes M_curr (all metadata) + LR indices
// Eliminates CPU-side L/R gather + PCIe upload of L_meta/R_meta
__kernel void hash_table_entries_lr(
    __global const uint* M_curr,       // [num_total_entries * 14] all metadata
    __global const uint* LR_pairs,     // [num_matches * 2] L,R indices
    __global uint* Y_out,              // [num_matches]
    __global uint* M_out,              // [num_matches * 14]
    const uint kmask,
    const uint num_matches,
    const uint num_total_entries)
{
    ...
    // Gather L/R meta directly from M_curr on GPU
    __private uint L[14], R[14];
    for(int i = 0; i < 14; i++) {
        L[i] = M_curr[P1 * 14 + i];
        R[i] = M_curr[P2 * 14 + i];
