/*
 * sha512_opt.cl — Optimized SHA-512 for GPU
 * 
 * Key optimization: circular buffer of 16 elements instead of w[80].
 * Reduces register pressure from 80 to 16 ulong registers per work-item.
 * Allows more concurrent work-items = better GPU utilization.
 */

__constant ulong SHA512_K_OPT[80] = {
...
__constant ulong SHA512_INIT_OPT[8] = {
	0x6a09e667f3bcc908UL, 0xbb67ae8584caa73bUL, 0x3c6ef372fe94f82bUL, 0xa54ff53a5f1d36f1UL,
	0x510e527fade682d1UL, 0x9b05688c2b3e6c1fUL, 0x1f83d9abfb41bd6bUL, 0x5be0cd19137e2179UL
};

ulong rotr64_opt(ulong x, int n) { return (x >> n) | (x << (64 - n)); }

/* Optimized SHA-512 block: uses 16-element circular buffer instead of w[80] */
void sha512_block_opt(__private ulong* msg, __private ulong* state)
{
	__private ulong w[16];
	...
	for(int i = 0; i < 80; i++) {
		// Compute next w value using circular buffer
		ulong w_next;
		if(i >= 16) {
			ulong w15 = w[(i - 15) & 15];
			ulong w2 = w[(i - 2) & 15];
			ulong s0 = rotr64_opt(w15, 1) ^ rotr64_opt(w15, 8) ^ (w15 >> 7);
			ulong s1 = rotr64_opt(w2, 19) ^ rotr64_opt(w2, 61) ^ (w2 >> 6);
			w_next = w[(i - 16) & 15] + s0 + w[(i - 7) & 15] + s1;
			w[i & 15] = w_next;
		} else {
			w_next = w[i];
		}
		
		ulong S1 = rotr64_opt(e, 14) ^ rotr64_opt(e, 18) ^ rotr64_opt(e, 41);
		ulong ch = (e & f) ^ ((~e) & g);
		ulong t1 = h + S1 + ch + SHA512_K_OPT[i] + w_next;
		ulong S0 = rotr64_opt(a, 28) ^ rotr64_opt(a, 34) ^ rotr64_opt(a, 39);
    ...
    const uint num_entries)
{
    ...
    for(int i = 0; i < 8; i++) state[i] = SHA512_INIT_OPT[i];
    sha512_block_opt(msg, state);
    
    __private ulong block2[16];
    for(int i = 0; i < 16; i++) block2[i] = 0;
    block2[15] = (ulong)112 * 8;
    sha512_block_opt(block2, state);
