/*
 * sha512_opt2.cl — Process 2 entries per work-item for better ILP
 */

__constant ulong SHA512_K2[80] = {
...
__constant ulong SHA512_INIT2[8] = {
	0x6a09e667f3bcc908UL, 0xbb67ae8584caa73bUL, 0x3c6ef372fe94f82bUL, 0xa54ff53a5f1d36f1UL,
	0x510e527fade682d1UL, 0x9b05688c2b3e6c1fUL, 0x1f83d9abfb41bd6bUL, 0x5be0cd19137e2179UL
};

ulong rotr64_2(ulong x, int n) { return (x >> n) | (x << (64 - n)); }

void sha512_block_2(__private ulong* msg, __private ulong* state)
{
	__private ulong w[80];
	for(int i = 0; i < 16; i++) w[i] = msg[i];
	for(int i = 16; i < 80; i++) {
		ulong s0 = rotr64_2(w[i-15], 1) ^ rotr64_2(w[i-15], 8) ^ (w[i-15] >> 7);
		ulong s1 = rotr64_2(w[i-2], 19) ^ rotr64_2(w[i-2], 61) ^ (w[i-2] >> 6);
		w[i] = w[i-16] + s0 + w[i-7] + s1;
	}
	ulong a=state[0], b=state[1], c=state[2], d=state[3];
	ulong e=state[4], f=state[5], g=state[6], h=state[7];
	for(int i = 0; i < 80; i++) {
		ulong S1 = rotr64_2(e, 14) ^ rotr64_2(e, 18) ^ rotr64_2(e, 41);
		ulong ch = (e & f) ^ ((~e) & g);
		ulong t1 = h + S1 + ch + SHA512_K2[i] + w[i];
		ulong S0 = rotr64_2(a, 28) ^ rotr64_2(a, 34) ^ rotr64_2(a, 39);
...
inline void compute_hash_pair(
    __global const uint* L_meta_in, __global const uint* R_meta_in,
    __global uint* Y_out, __global uint* M_out,
    const uint kmask, const uint gid)
{
    ...
    for(int i = 0; i < 8; i++) state[i] = SHA512_INIT2[i];
    sha512_block_2(msg, state);
    
    __private ulong block2[16];
    for(int i = 0; i < 16; i++) block2[i] = 0;
    block2[15] = (ulong)112 * 8;
    sha512_block_2(block2, state);
...
/* Process 2 entries per work-item for better ILP */
__kernel void hash_table_entries_opt(
    __global const uint* L_meta_in,
    __global const uint* R_meta_in,
    __global uint* Y_out,
    __global uint* M_out,
    const uint kmask,
    const uint num_entries)
{
    ...
    // Process first entry
    compute_hash_pair(L_meta_in, R_meta_in, Y_out, M_out, kmask, gid);
    
    // Process second entry (ILP: independent computation)
    if(gid + 1 < num_entries) {
        compute_hash_pair(L_meta_in, R_meta_in, Y_out, M_out, kmask, gid + 1);
    }
}
